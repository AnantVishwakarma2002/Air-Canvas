...
while True:
    ...
    if len(lmList)!=0:
        ...
        x1,y1 = lmList[8][1:]    #Coordinates of the tip of our index finger (lmlist mein 1st and 2nd index of the 8h index are the x and y coordinates)
        x2,y2 = lmList[12][1:]   # #Coordinates of the tip of our middle finger (lmlist mein 1st and 2nd index of the 8h index are the x and y coordinates)
    ...
    imageGray = cv2.cvtColor(imageCanvas,cv2.COLOR_BGR2GRAY)    #Converting the imagecanvas to an gray image

    _,imageInverse = cv2.threshold(imageGray,50,255,cv2.THRESH_BINARY_INV)    #Convert the gray image to a binary and inversed image
    imageInverse = cv2.cvtColor(imageInverse,cv2.COLOR_GRAY2BGR)    #WE can't add a gray image to a coloured image
